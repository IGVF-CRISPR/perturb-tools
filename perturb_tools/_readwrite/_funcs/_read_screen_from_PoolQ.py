...
from ..._framework._supporting_functions._print_screen_object import (
    ...
    for n, line in enumerate(quality_file):
        ...
        if not line is None:
            if ":" in line[0]:
                ...
                _read_count_for_df(PoolQ_QualityDict["bc_readcounts"], n, line)
            elif len(line) == 2:
                _read_count_for_df(PoolQ_QualityDict["common_barcodes"], n, line)
            else:
                if verbose:
...
def _assemble_PoolQ_Quality_Dict():
    return _create_EmptyDict(keys=["metadata", "bc_readcounts", "common_barcodes"])
...
def _assemble_pandas_dict(filepaths: Sequence[str], keys=False, sep="\t", **kwargs):
    """"""
...
def _load_parse_PoolQ_counts_df(
    PoolQ_OutsDict, ScreenDict=None, counts_keys=["counts", "lognormalized-counts"]
):
    """
    ...
    ScreenDict["X"] = (
        df_dict["counts"].drop(["Row Barcode", "Row Barcode IDs"], axis=1).values
    )
    ScreenDict["guides"] = df_dict["counts"][["Row Barcode", "Row Barcode IDs"]]
    ScreenDict["guides"].columns = ["barcode", "barcode_id"]
    ScreenDict["layers"]["lognorm_counts"] = df_dict["lognormalized-counts"].drop(
        ["Row Barcode", "Row Barcode IDs"], axis=1
    )
    ScreenDict["condit"] = pd.DataFrame(
        ScreenDict["layers"]["lognorm_counts"].columns, columns=["Condition"]
    )
...
def _compose_condit_mp(
    ...
    ScreenDict["condit_p"] = _read_poolq_counts_dfs(
        PoolQ_OutsDict, ["correlation"], index_col=0
    )

    return ScreenDict


def _compose_uns_from_quality_file(
    ScreenDict, PoolQ_OutsDict, poolq_outs_path, out_path
):
...
def _read_screen_from_PoolQ(poolq_outs_path, analaysis_out_path="wokrbook.xlsx"):
    ...
    ScreenDict = _load_parse_PoolQ_counts_df(
        PoolQ_OutsDict, ScreenDict=False, counts_keys=["counts", "lognormalized-counts"]
    )
    ScreenDict = _compose_condit_mp(
        ScreenDict,
        PoolQ_OutsDict,
        counts_keys=["barcode-counts", "unexpected-sequences"],
    )
    ScreenDict = _compose_uns_from_quality_file(
        ScreenDict, PoolQ_OutsDict, poolq_outs_path, out_path=analaysis_out_path
    )

    return ScreenDict


def read_PoolQ(adata: AnnData, path, metadata=False, merge_metadata_on="Condition"):
    """Read poolQ."""
    adata.uns["_PoolQ_outpath"] = path
    adata.uns["_PoolQScreenDict"] = _read_screen_from_PoolQ(adata.uns["_PoolQ_outpath"])

    for key, value in _update_dict(adata.uns["_PoolQScreenDict"]).items():
        adata.__setattr__(key, value)

    if metadata:
        adata.obs = adata.obs.merge(pd.read_csv(metadata), on=merge_metadata_on)

    _print_screen_object(adata)
