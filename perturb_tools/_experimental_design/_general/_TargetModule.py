...
from ._get_screen_target_features import (
    _gene_from_gtf,
    _get_feature,
    _get_gene_body_bounds,
)
...
class _ScreeningTarget:
    ...
    def get_exons(self):
        ...
        self.exon_df["exon_length"] = self.exon_df["End"].astype(int) - self.exon_df[
            "Start"
        ].astype(int)

    def build_guide_library(
        self, exons=True, feature=None, PAM="NGG", filter_BsmbI=True, widen=False
    ):
        """
        ...
        if exons == True:
            ...
            if hasattr(self, "feature_df"):
                ...
                self.regions_df = self.get_features(self.gene_df, feature)
        if widen:
            print(
        ...
        forward_pams, reverse_pams, chrom_seq = _query_region_for_PAM(
            self.regions_df, self.ref_seq_path, PAM=PAM
        )

        print("PAMs, Chromosome Seqeunce isolated")

        self.guide_df = _make_guide_library_df(
            forward_pams, reverse_pams, self.regions_df, chrom_seq
        )
        ...
        self.guide_df, self.BsmbI_guide_df = _scan_for_BsmbI(
            self.guide_df, filter_BsmbI
        )
