...
def _transform_guide_spanning_dict_to_guide_df_addition(GuidesSpanning, guide_df):
    ...
    for strand in ["+", "-"]:
        ...
        for protospacer, value in GuidesSpanning[strand].items():
            ...
            GuidePosition[protospacer] = [strand, value[0], value[1]]

    df_ = (
        pd.DataFrame(GuidePosition, index=["strand", "Start", "End"])
        .T.reset_index()
        .rename({"index": "protospacer"}, axis=1)
    )

    return guide_df.merge(df_, on="protospacer", how="outer")
    ...
    for target in GeneDict.keys():
        ...
        for spacer in target_guides.protospacer.values:
            try:
                ...
                GuidesSpanning["+"][spacer] = (int(guide_start), int(guide_end))
            except:
                try:
